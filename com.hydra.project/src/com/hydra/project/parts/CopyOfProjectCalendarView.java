...
public class CopyOfProjectCalendarView {
	...
	private Text text_Summe_Stunden;
	...
	private static final String STUNDEN					= "P061.003.014";
...
//	private static List arrayListVorgang =new ArrayList<String>();
	...
	@Inject
	public CopyOfProjectCalendarView() {
...
//#######################################################		
		...
		cmpKalenderOben.setLayout(new GridLayout(8, false));
		
		Label lblNewLabel = new Label(cmpKalenderOben, SWT.NONE);
		...
		lblVorgang.setLayoutData(new GridData(SWT.RIGHT, SWT.CENTER, false, false, 1, 1));
		lblVorgang.setText("Vorgang");
		
		comboVorgang = new Combo(cmpKalenderOben, SWT.READ_ONLY);
		GridData gd_combo_Vorgang = new GridData(SWT.FILL, SWT.CENTER,true, false, 1, 1);
		gd_combo_Vorgang.widthHint = 30;
		comboVorgang.setLayoutData(gd_combo_Vorgang);
		...
		new Label(cmpKalenderOben, SWT.NONE);
		...
		txtStunden.setBounds(518, 118, 156, 21);
		...
		GridData gd_btnVorgangLoeschen = new GridData(SWT.LEFT, SWT.CENTER, false, false, 1, 1);
		gd_btnVorgangLoeschen.widthHint = 127;
		btnVorgangLoeschen.setLayoutData(gd_btnVorgangLoeschen);
		btnVorgangLoeschen.setText("Vorgang l\u00F6schen");

		final Button btnAlleAnzeigen = new Button(cmpKalenderOben, SWT.TOGGLE);
		GridData gd_btnAlleAnzeigen = new GridData(SWT.LEFT, SWT.CENTER, false, false, 1, 1);
		gd_btnAlleAnzeigen.widthHint = 126;
		btnAlleAnzeigen.setLayoutData(gd_btnAlleAnzeigen);
		...
		lblDatum.setLayoutData(new GridData(SWT.RIGHT, SWT.CENTER, false, false, 1, 1));
		lblDatum.setText("Datum");
		textDatum = new Text(cmpKalenderOben, SWT.BORDER);
		textDatum.setEnabled(false);
		textDatum.setEditable(false);
		textDatum.setLayoutData(new GridData(SWT.FILL, SWT.CENTER, true, false, 1, 1));
		textDatum.setText(txtAktuelleDatum);
//################################################
		
		
		Composite cmpKalenderUnten = new Composite(cmpKalender, SWT.NONE);
		cmpKalenderUnten.setLayoutData(BorderLayout.SOUTH);
		
		Button btnUebernehmenKalender = new Button(cmpKalenderUnten, SWT.NONE);
		btnUebernehmenKalender.setBounds(697, 29, 91, 25);
		btnUebernehmenKalender.setText("\u00DCbernehmen");
...
//						}
	 ...
	 */
	private void updateTreeItem(String parameter, String value){
		MyTreeItem myFoundTreeItem = null;
	...
	private void updateValue(MyTreeItem myTreeItem){
		...
		case ARBEITSDAUERDONNERSTAG:
			txtArbeitsdauerDonnerstag.setText(myTreeItem.getVariablenWert());
			break;
		...
		case ARBEITSDAUERSONNTAG:
			...
			break;
		case AKTIVETASKS:
			btnNurAktive.setSelection(Boolean.valueOf(myTreeItem.getVariablenWert()));
			break;
		case VORGANG:
			...
			comboVorgang.setItems(stringArray);
			comboVorgang.setText(myTreeItem.getVariablenWert());
			break;	
		case DATUM:
		...
		case STARTTERMIN:
  			if (myTreeItem.getObject()== null) {		//kein Datum vorhanden
   				Date date = new Date();					//nehme aktuelles Datum
   				myTreeItem.setObject(date);		//da Zeitformat nicht umgewandelt werden kann aus String wird hier der Wert direkt geholt
   				dateChooserStart.setSelectedDate((Date) myTreeItem.getObject());
   			}else{
   				dateChooserStart.setSelectedDate((Date) myTreeItem.getObject());
   			}
			break;
		case ENDTERMIN:
  			if (myTreeItem.getObject()== null) {		//kein Datum vorhanden
   				Date date = new Date();					//nehme aktuelles Datum
   				myTreeItem.setObject(date);		//da Zeitformat nicht umgewandelt werden kann aus String wird hier der Wert direkt geholt
   				dateChooserEnd.setSelectedDate((Date) myTreeItem.getObject());
   			}else{
   				dateChooserEnd.setSelectedDate((Date) myTreeItem.getObject());
   			}
			break;
	...
	public static void setTasks(ArrayList<MyTasksModel> tasks) {
		CopyOfProjectCalendarView.tasks = tasks;
	}
	...
	public static void setDateChooserStart(DateChooser dateChooserStart) {
		CopyOfProjectCalendarView.dateChooserStart = dateChooserStart;
	}
	...
	public static void setDateChooserEnd(DateChooser dateChooserEnd) {
		CopyOfProjectCalendarView.dateChooserEnd = dateChooserEnd;
	}
	...
	public static void setProjektDauerInMonaten(int projektDauerInMonaten) {
		CopyOfProjectCalendarView.projektDauerInMonaten = projektDauerInMonaten;
	}
	...
	public static void setDateDatum(Date dateDatum) {
		CopyOfProjectCalendarView.dateDatum = dateDatum;
	}
