 ...
 *
 */
public class MyTreeItem2 implements Cloneable {
...
//	private boolean dateiItem;				//true wenn zweitoberster Knoten
	...
	private List<CustomizeData> custDatas;		//Enhält die Einstellungen von Tabellen
	...
	//Beziehungen
	private MyTreeItem2 parent;
	private String parentUUID;
	private MyTreeItem2 preChild;			//vorhergehende Kind
	private MyTreeItem2 postChild;			//nachfolgende Kind
	...
	protected List <MyTreeItem2> children;	//Liste der Kinder eines Knoten
...
//	@Inject
	...
	private void setup(){
		...
		children = new ArrayList<MyTreeItem2>();
//		treePath = new ArrayList<String>();
	...
	@Override
	public MyTreeItem2 clone(){
	    try
		    {
	    		MyTreeItem2 myTreeItem = (MyTreeItem2) super.clone();
	 ...
	 */
	public void Rename (MyTreeItem2 myTreeItem){
		this.dirty = true;
	 ...
	 */
	public void addChild(MyTreeItem2 myTreeItem) {
		this.dirty = true;

		int anzahl=0;
		if (children == null){
			children = new  ArrayList<MyTreeItem2>(5);
	 ...
	 */
	public void setChild(MyTreeItem2 myTreeItem) {
		this.dirty = true;
	...
	public boolean removeChild(MyTreeItem2 myTreeItem) {
		this.dirty = true;
		int anzahl=0;
		if (children != null) {
			for (int i = 0; i < children.size(); i++) {
	...
	public List<MyTreeItem2> getChildren() {
		 try {
	...
	/**
	 * Holt die benutzerdefinierten Tabelleneinstellungen
	 * @return custDatas Die Tabelleneinstellungen
	 */
	...
	public void shiftMyTreeItemDownwards() {
		...
		//tauscht zwei Kinder gegeneinander aus
			MyTreeItem2 child1=this.getParent().children.get(getIndex());
			MyTreeItem2 child2=this.getParent().children.get(getIndex()+1);
	...
	public void shiftMyTreeItemUpwards() {
		this.dirty = true;
		//tauscht zwei Kinder gegeneinander aus
			MyTreeItem2 child1=this.getParent().children.get(this.getIndex());
			MyTreeItem2 child2=this.getParent().children.get(this.getIndex()-1);
			child1.setIndex(child1.getIndex()-1);
			child2.setIndex(child2.getIndex()+1);
			this.getParent().children.set(child1.getIndex(), child1);
			this.getParent().children.set(child2.getIndex(), child2);
	 ...
	 * @return the preChild
	 */
	public MyTreeItem2 getPreChild() {
		MyTreeItem2 parent = this.getParent();
		if(this.getIndex()==0){
			return null;
		}else {
			if (!(parent.getChildren()==null)){
				MyTreeItem2 preChild = parent.children.get(this.getIndex()-1);
				return preChild;
			}
		}
		return null;
	 ...
	 */
	public MyTreeItem2 getPostChild() {
		MyTreeItem2 parent = this.getParent();
		if(!(parent.getChildren()==null)){
			if (parent.getChildren().size()-1 > this.getIndex()){
				MyTreeItem2 postChild = parent.getChildren().get(this.getIndex()+1);
				return postChild;
			}
		}
		return null;
	 ...
	 */
	public void replaceMyTreeItem(MyTreeItem2 myOldTreeItem, MyTreeItem2 myNewTreeItem) {
	 ...
	 */
	public Integer findMyTreeItemChild(MyTreeItem2 myTreeItem) {
		Integer index = -1;
	 ...
	 */
	public MyTreeItem2 findMyTreeItem(String uuid) {
		MyTreeItem2 myTreeItem = new MyTreeItem2();
		myTreeItem=null;
		if(this.isHasChildren()){			//wenn Kinder vorhanden dann durchsuchen
			for (int i=0; i<children.size();i++){
...
//				" "+
	 ...
	 */
	public MyTreeItem2 getParent() {
		MyTreeItem2 myParentTreeItem = new MyTreeItem2();
...
//		}
	 ...
	 */
	public void setPreChild(MyTreeItem2 preChild) {
		this.dirty = true;
	 ...
	 */
	public void setPostChild(MyTreeItem2 postChild) {
		this.dirty = true;
	 ...
	 */
	public void setChildren(List<MyTreeItem2> children) {
		this.dirty = true;
		this.children = children;
	}
	
	/**
	 * Setzt die benutzerdefinierten Tabelleneinstellungen
	 * @param custDatas die Tabelleneinstellungen
	 */
	public void setCustDatas(List<CustomizeData> custDatas) {
		this.dirty = true;
	 ...
	 */
	public void setParent(MyTreeItem2 parent) {
